...
if page == "Journal de Bord":
    ...
    strat_list = ["Aucune", "FAILED", "Reject vwap", "Double Top PMH", "Double Top DAY", "Double Top Intraday", "Open Drop", "Zone Morte", "Open Push"]
        ...
        if t_ticker:
            with st.spinner(f"Traitement de {t_ticker}..."):
                ...
                new_row.update({
                    ...
                    "Open to VWAP %": m_ov, "Max Dev Vwap %": m_md, 
                    "HOD to LOD Drop %": stats.get("HOD to LOD Drop %", 0.0),
    ...
    # --- 📂 HISTORIQUE (Bouton Sauvegarder en haut à droite) ---
    ...
    # --- 🔍 TESTEUR DE TICKER (Alignement du bouton corrigé) ---
    st.divider()
    ...
    # Ajout d'un container invisible pour aligner verticalement le bouton avec le texte
    test_c3.markdown("<div style='height: 28px;'></div>", unsafe_allow_html=True)
