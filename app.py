...
# 🟢 "Aucun" est de retour dans la liste
...
def get_stat(target_ticker, target_date, s1, s2, news_val, filling_val, entry_time, comment_text):
    ...
    rvol_val = round(pm_df['v'].sum() / avg_basis, 2) if not pm_df.empty else 0
...
# 🟢 SUPPRESSION DU ST.FORM pour permettre l'interactivité temps réel
...
with c_fill:
    # 🟢 Multiselect vide par défaut, mais "Aucun" est une option
    fill_sel = st.multiselect("Filling Type", LISTE_FILLING_DEFAUT)
    custom_fill = ""
    # 🟢 APPARAÎT INSTANTANÉMENT car on n'est plus dans un formulaire
    if "AUTRE..." in fill_sel:
        custom_fill = st.text_input("👉 Précisez le type (ex: S-3, 10-K)")
...
if submitted and t_ticker:
    ...
    # Nettoyage de la sélection multi
    filling_list = [f for f in fill_sel if f not in ["AUTRE...", "Aucun"]]
    if custom_fill:
        filling_list.append(custom_fill)
    
    filling_final = ", ".join(filling_list) if filling_list else "Aucun"
    
    res, msg = get_stat(t_ticker, t_date, s1, s2, news_final, filling_final, entry_time_final, comment)
    ...
    if res is not None:
        ...
        if "PM RVOL" in cols:
            cols.insert(cols.index("Float (Shares)") + 1, cols.pop(cols.index("PM RVOL")))
            final_df = final_df[cols]

        final_df.sort_values(by="Date", ascending=True).to_excel(EXCEL_FILE, index=False)
        st.success(f"✅ {t_ticker} ajouté au journal !")
        st.rerun() # Pour vider les champs après l'ajout
    else: st.error(msg)
...
if os.path.exists(EXCEL_FILE):
    ...
    st.data_editor(df_view, num_rows="dynamic", use_container_width=True, hide_index=True)
